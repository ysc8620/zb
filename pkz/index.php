...
$share_title = '贫困证图片生成器';
$name = htmlspecialchars(trim($_GET['name']));
$share_title = ($name)?"【{$name}】的贫困证":"贫困证图片生成器,由{$from_title}提供";
$share_link = "http://{$_SERVER[HTTP_HOST]}".'/pkz/?name='.$name.'&id='.$_GET['id'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>贫困证图片生成器-<?php echo $from_title;?></title>
...
<?php if($_GET['name']){?>
<div class="wrapper">
	<img src="pkz.php?name=<?=$_GET['name']?>" width="100%"/>
</div>
<?php }else{ ?>
